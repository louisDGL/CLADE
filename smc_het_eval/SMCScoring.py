...
import io
import sys
# NOTE: metric_behavior is imported lazily inside calculate3 (the only user of `mb`)
# to avoid a circular import (metric_behavior does `from ...SMCScoring import *`),
# which otherwise breaks under the dotted package-style import paths in Python 2.
from functools import reduce
from smc_het_eval.scoring_harness_optimized import *
from smc_het_eval.permutations import *
import gc
...
def calculate3(pred_ccm, pred_ad, truth_ccm, truth_ad, method="sym_pseudoV", weights=None, verbose=False, pseudo_counts=True, full_matrix=True, in_mat=2):
    ...
    """
    import smc_het_eval.metric_behavior as mb  # lazy import to avoid circular import
...
def filterFPs(x, mask):
    ...
    #   4 - the array == "shrunk" to discard the difference between the old size and the new size
...
def get_bad_ccm(nssms, scenario='OneCluster'):
    ...
    dim = int(nssms)
    if scenario == 'OneCluster':
        return np.ones([dim, dim], dtype=np.int8)
    elif scenario == 'NCluster':
