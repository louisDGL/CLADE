    ...
    if func is None:
        ...
        for m in functions:
          #  print("m", m, func_dict[m])
            gc.collect()
            if m == 'pseudoV' or m == 'sym_pseudoV' or m == 'js_divergence':
...
def construct_relative_matrix(om, pred, truth, mode="ancestor"):
    ...
    for i in range(truth.shape[1]):
        for j in range(pred.shape[1]):
            ...
            if mode == "ancestor" or mode == "cousin":
...
def calculate3A_final(om, ad_pred, ad_truth, modification=None, rnd=1e-50):
    ...
    if modification is None:
        pred = ad_pred
        truth = ad_truth
    elif modification == "transpose":
        pred = ad_pred.T
        truth = ad_truth.T
    elif modification == "cousin":
